...
export async function POST(request: NextRequest) {
  try {
    const { items, projectDescription, priceScale, location, clientSessionData } = await request.json()
    ...
    if (clientSessionData && typeof clientSessionData === 'object') {
      console.log('[v0] Server: Syncing with client session data:', clientSessionData)
      rateLimiter.syncWithClientSession(clientSessionData)
    }
    const remainingCalls = rateLimiter.getRemainingCalls()
    
    if (remainingCalls === 0) {
      console.warn("[v0] Server: API rate limit exceeded")
    ...
    // Limit items to remaining API calls to prevent exceeding limit
    const itemsToProcess = items.slice(0, remainingCalls)
    if (itemsToProcess.length < items.length) {
      console.warn(`[v0] Server: Limiting search to ${itemsToProcess.length} items due to API rate limit (${remainingCalls} calls remaining)`)
    }

    console.log("[v0] Server: Starting SerpAPI pricing search for", itemsToProcess.length, "items")
    ...
    for (const item of itemsToProcess) {
      try {
        ...
        if (!rateLimiter.recordCall()) {
          console.warn("[v0] Server: API rate limit reached during processing")
          results.push({
            itemName: item.name,
        ...
        const searchQuery = createSearchQuery(item.name)
        console.log("[v0] Server: Searching for:", item.name, "with query:", searchQuery)
        ...
        console.log('[v0] Server: Full API URL:', `https://serpapi.com/search.json?${params}`)

        const response = await fetch(`https://serpapi.com/search.json?${params}`)

        if (!response.ok) {
          console.error("[v0] Server: Google Shopping API request failed:", response.status, response.statusText)
          results.push({
            itemName: item.name,
            quantity: item.quantity,
            price: null,
        ...
        console.log("[v0] Server: Google Shopping API Response status:", data.search_metadata?.status)
        console.log("[v0] Server: Raw shopping results found:", data.shopping_results?.length || 0)

        if (data.shopping_results && data.shopping_results.length > 0) {
          console.log('[v0] Server: Raw shopping results found:', data.shopping_results.length)
          console.log('[v0] Server: First 3 raw products:')
          data.shopping_results.slice(0, 3).forEach((p: GoogleShoppingProduct, i: number) => {
            console.log(`[v0] ${i + 1}. ${p.title} - $${p.price} - Source: ${p.source || 'unknown'} - Product Link: ${p.product_link}`)
          })
          
          // Use AI to select the top 3 products
          const top3Products = await selectTop3ProductsWithAI(data.shopping_results, item, projectDescription, priceScale, location)
          
          console.log("[v0] Server: AI selection result for", item.name, ":", top3Products.length, "products selected")
          
          if (top3Products && top3Products.length > 0) {
            console.log(
              "[v0] Server: AI SELECTED top 3 products for:",
              item.name,
              "Found",
              top3Products.length,
              "products"
            ...
            // Return only the top result, the compare functionality will fetch others via separate API
            const topProduct = top3Products[0]
            const priceValue = topProduct.extracted_price || parsePrice(topProduct.price) || 0
            const productLink = topProduct.product_link || topProduct.link || ''
            ...
            results.push({
              ...
              quantity: item.quantity,
              price: priceValue,
              totalCost: priceValue * item.quantity,
              ...
              condition: topProduct.condition,
              hasMoreOptions: top3Products.length > 1, // Indicate if more options are available
            })
          } else {
            console.log("[v0] Server: AI found no suitable products for:", item.name)
            results.push({
              itemName: item.name,
              quantity: item.quantity,
              price: null,
          ...
          console.log("[v0] Server: No products found for:", item.name)
          results.push({
            itemName: item.name,
            quantity: item.quantity,
            price: null,
        ...
        console.error("[v0] Server: Error searching for", item.name, ":", error)
        results.push({
          itemName: item.name,
          quantity: item.quantity,
          price: null,
    ...
    console.log(
      "[v0] Server: Pricing search completed. Found prices for",
    ...
    console.log('[v0] Server: Final API stats before response:', finalStats)
